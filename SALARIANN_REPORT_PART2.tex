...
\textbf{Responsibilities:}
\begin{itemize}
    \item Aggregate salary data across roles, companies, and experience levels to identify market trends and growth trajectories.
    \item Implement a suitability scoring algorithm that ranks jobs based on user skills, experience, and career goals.
    \item Generate personalized job recommendations using collaborative filtering and content-based similarity metrics.
    \item Provide company analytics including average salaries, review ratings, growth rates, and tech stack preferences.
\end{itemize}
...
\textbf{Responsibilities:}
\begin{itemize}
    \item Implement JWT-based authentication with secure token generation and validation.
    \item Store user profiles including personal information, career preferences, lifestyle choices, and saved jobs.
    \item Manage user sessions and provide logout functionality with token revocation.
    \item Track user interactions (job views, applications, saves) for analytics and recommendation personalization.
\end{itemize}
...
\begin{itemize}
    \item Mapping API field names to internal field names using predefined transformation rules.
    \item Handling missing or null values through intelligent defaults.
    \item Detecting and merging duplicate job listings from multiple sources using fuzzy string matching.
\end{itemize}

\subsection{Affordability-Driven Ranking}

Rather than ranking jobs by recency or popularity (as traditional job boards do), Salariann ranks results by affordability score, ensuring that the most financially viable opportunities appear first.

\subsection{Iterative Recommendation Refinement}

The system implements a feedback loop where user interactions (job views, applications, saves) are collected and used to refine recommendation models. Over time, the system learns user preferences and improves recommendation relevance.
...
\begin{enumerate}
    \item \textbf{User Authentication}: The user navigates to the Salariann home page and signs in using email and password. The frontend sends credentials to the backend \texttt{/auth/login} endpoint.
    
    \item \textbf{Profile Setup}: On first login, the user completes a profile questionnaire specifying lifestyle preferences, desired salary range, preferred cities, and tech stack expertise.
    
    \item \textbf{Job Search Initiation}: The user enters a search query and selects one or more cities. The frontend sends a GET request to \texttt{/api/live/jobs/city/\{city\}}.
    
    \item \textbf{Real-Time Job Aggregation}: The backend receives the request and calls the JSearch API with the search query and city filter.
    ...
    \item \textbf{Affordability Scoring}: The backend computes an affordability score for each job using the user's salary preferences.
